...
*II. OPEN DATA CENSUS AGRO 1988 AND ACCOMODATE NAMES
/*
...
clean CAPITANSARMIENTO CAPSARMIENTO `pr'
clean CORONELDORREGO CNELDORREGO  `pr'
clean CORONELPRINGLES CNELPRINGLES `pr'
clean CORONELSUAREZ CNELSUAREZ `pr'
clean CORONELROSALES CNELROSALES `pr'
clean EXALTACIONDELACRUZ EDELACRUZ `pr'
clean GENERALALVARADO GRALALVARADO `pr'
clean GENERALALVEAR GRALALVEAR `pr'
clean GENERALARENALES GRALARENALES `pr'
clean GENERALBELGRANO GRALBELGRANO `pr'
clean GENERALPAZ GRALPAZ `pr'
clean GENERALPUEYRREDON GRALPUEYRREDON `pr'
clean GENERALRODRIGUEZ GRALRODRIGUEZ `pr'
clean GENERALSANMARTIN GRALSANMARTIN  `pr'
clean GENERALSARMIENTO GRALSARMIENTO `pr'
clean GENERALVILLEGAS GRALVILLEGAS `pr'
clean GENERALALVARADO GRALALVARADO `pr'
clean GENERALALVEAR GRALALVEAR `pr'
clean GENERALARENALES GRALARENALES `pr'
clean LEANDRONALEM LNALEM `pr'
clean SANANTONIODEARECO SADEARECO `pr'
clean SANANDRESDEGILES SANDRESDEGILES `pr'
clean ADOLFOGONZALEZCHAVES ADOLFOGCHAVES `pr'
clean CORONELDEMARINELROSALES CORONELROSALES `pr'
clean GENERALLAMADRID GRALLAMADRID `pr'
clean GENERALLASHERAS GRALLASHERAS `pr'
clean GENERALLAVALLE GRALLAVALLE `pr'
clean GENERALPINTO GRALPINTO `pr'
clean GENERALGUIDO GRALGUIDO `pr'
clean GENERALJUANMADARIAGA GRALMADARIAGA `pr'
clean GENERALVIAMONTE GRALVIAMONTE `pr'
clean ARRECIFES BMEMITRE `pr'
/*
cambio de nombre en 1997
https://es.wikipedia.org/wiki/Partido_de_Arrecifes
*/

drop if distmerge=="INDETERMINADO"
/*
INDETERMINADO es el "nombre" del partido en la publicacion original del censo
corresponde a 10 explotaciones , en total 12,901 ha.
*/
...
clean DRMANUELBELGRANO GRALMANUELBELGRANO `pr'
...
clean VINCHINA GRALSARMIENTO `pr'
...
clean CORONELFELIPEVARELA CNELFVARELA `pr'
clean GENERALANGELVPENALOZA GRALAVPENALOZA `pr'
clean GENERALBELGRANO GENERALMANUELBELGRANO `pr'
clean GENERALJUANFQUIROGA GRALJFQUIROGA `pr'
clean GENERALLAMADRID GRALLAMADRID `pr'
clean GENERALOCAMPO GRALOCAMPO `pr'
clean GENERALSANMARTIN GRALSANMARTIN `pr'
clean ROSARIOVERAPENALOZA RVPENALOZA `pr'
...
clean LEANDRONALEM LEANDONALEM `pr'
/*
...
clean LIBERTADORGENERALSANMARTIN LIBGRALSANMARTIN `pr'
clean GENERALJOSEDESANMARTIN GRALJOSEDESANMARTIN `pr'
...
local pr = "SANTAFE"
clean 9DEJULIO NUEVEDEJULIO `pr'
clean GENERALLOPEZ GRALLOPEZ `pr'
clean GENERALOBLIGADO GRALOBLIGADO `pr'
...
clean GENERALTABOADA GRALTABOADA `pr'
...
sort provm distm
list provm distm _merge if _merge!=3

assert _merge!=2 if provmerge!="CAPITALFEDERAL" & provmerge!="TIERRADELFUEGO"
drop if provmerge =="CAPITALFEDERAL" | provname=="TIERRADELFUEGO"
...
save_data "..\temp\ag1988_ipums.dta", replace key(geolev2)
