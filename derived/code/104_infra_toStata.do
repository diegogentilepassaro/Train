...
import dbase using "..\temp\\hypo_LCP_total_MST.dbf", clear case(lower)
ren *, lower
keep length_met geolevel2
collapse (sum) length_met, by(geolevel2)
gen hypo_LCP_total_MST_kms = length_met / 1000
drop length_met
tempfile hypo_LCP_total_MST
save `hypo_LCP_total_MST', replace
...
import dbase using "..\temp\\hypo_EUC_total_MST.dbf", clear case(lower)
ren *, lower
keep length_met geolevel2
collapse (sum) length_met, by(geolevel2)
gen hypo_EUC_total_MST_kms = length_met / 1000
drop length_met
tempfile hypo_EUC_total_MST
save `hypo_EUC_total_MST', replace
...
*assert _merge==3
drop _merge
merge 1:1 geolevel2 using `hypo_LCP_total_MST'
*assert _merge==3
drop _merge
...
*assert _merge==3
drop _merge
merge 1:1 geolevel2 using `hypo_EUC_total_MST'
*assert _merge==3
drop _merge
...
foreach var of var statusLP_1 statusLP_2 statusLP_3 status79_1 studied_0 studied_1 ///
	...
	hypo_LCP_MST_kms hypo_LCP_total_MST_kms hypo_LCP_plain_MST_kms hypo_EUC_MST_kms hypo_EUC_total_MST_kms ///
