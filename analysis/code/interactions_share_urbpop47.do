    ...
    *** 1960 base outcomes
	...
	foreach depvar in log_pop log_urbpop share_urbpop log_share_urbpop {
       ...
       run_IV_regression, depvar(chg_`depvar'_91_60) ///
		    ...
		    table_name(IV_both_int_chg_`depvar'_91_60_pav_and_grav)
	}
    ...
    *** 1970 base outcomes
	...
	foreach depvar in log_pop {
       ...
       run_IV_regression, depvar(chg_`depvar'_91_70) ///
		    ...
		    table_name(IV_both_int_chg_`depvar'_91_70_pav_and_grav)
	}
	...
	*** Employment
	...
	foreach depvar in log_agareatot_ha log_agnexp {
       ...
       run_IV_regression, depvar(chg_`depvar'_88_60) ///
	        ...
	        roads_var(chg_pav_and_grav_86_70) ///
		    trains_var(chg_tot_rails_86_70) ///
		    table_name(IV_both_int_chg_`depvar'_pav_and_grav)
	}
...
program run_IV_regression
	...
	eststo: qui reghdfe `depvar' `geo_vars' c.pop_1960#c.`roads_var' c.`roads_var' ///
	    c.pop_1960#c.`trains_var' c.`trains_var', absorb(provname)
		
	eststo: qui reghdfe `depvar' `geo_vars' c.pop_1960#c.`roads_var' c.`roads_var' ///
	    c.pop_1960#c.`trains_var' c.`trains_var' ///
	    `baseline_depvar', absorb(provname)
	
	eststo: qui ivreghdfe `depvar' `geo_vars' ///
	    (c.`roads_var' c.pop_1960#c.`roads_var' ///
		c.`trains_var' c.pop_1960#c.`trains_var' = ///
		c.hypo_EUC_total_MST_kms c.pop_1960#c.hypo_EUC_total_MST_kms ///
		c.studied_larkin c.pop_1960#c.studied_larkin), absorb(provname)
	...
	eststo: qui ivreghdfe `depvar' `geo_vars' ///
		...
		c.`trains_var' c.pop_1960#c.`trains_var' = ///
		c.hypo_EUC_total_MST_kms c.pop_1960#c.hypo_EUC_total_MST_kms ///
		c.studied_larkin c.pop_1960#c.studied_larkin) ///
		`baseline_depvar', absorb(provname)
	qui estadd local F_stat_fs = round(e(cdf), 0.0001)
	    ...
	    (c.`roads_var' c.pop_1960#c.`roads_var' ///
		c.`trains_var' c.pop_1960#c.`trains_var' = ///
		c.hypo_LCP_total_MST_kms c.pop_1960#c.hypo_LCP_total_MST_kms ///
		c.studied_larkin c.pop_1960#c.studied_larkin), absorb(provname)
	qui estadd local F_stat_fs = round(e(cdf), 0.0001)
	...
	esttab * using "../output/`table_name'.tex", replace compress ///
		...
		order(`trains_var' c.pop_1960#c.`trains_var' ///
		`roads_var' c.pop_1960#c.`roads_var' ///
		`baseline_depvar') label nonotes ///
	    keep(`trains_var' c.pop_1960#c.`trains_var' ///
		`roads_var' c.pop_1960#c.`roads_var' `baseline_depvar') ///
